...
We attempted multiple network designs, each of which is evaluated in Section \ref{sec:exp}. We begin with the Multi-Unit Network Design in Figure \ref{fig:design_multiunit}.

\begin{figure}
\input{multiunit_orig}
\caption{The initial multi-unit network design for 3 allies and 3 enemies. $r$ and $\theta$ represent relative distance and angle. $s$ represents a score in the range $[0,1]$, and $f$ is a flip.}
\label{fig:design_multiunit}
\end{figure}

\begin{figure}
\input{individualcontroller}
\caption{The initial individual controller network design.}
